...
class InnerNode extends BPlusNode {
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
    ...
    BPlusNode nextNode = getChild(i);
    // recursively put down tree
    Optional<Pair<DataBox, Integer>> putResult = nextNode.put(key, rid);

    // if split occurs in child
    if (putResult.isPresent()) {
      Pair<DataBox, Integer> splitResult = putResult.get();
      // insert key, child here
      this.keys.add(i, splitResult.getFirst());
      this.children.add(i+1, splitResult.getSecond());
      ...
      // split
        ...
        DataBox pushUp = this.keys.get(order);

        for (int m = order+1; m <= order*2; m++) {
          newKeys.add(this.keys.get(m));
          newChildren.add(this.children.get(m));
        }
        newChildren.add(this.children.get(order*2+1));
        this.keys = this.keys.subList(0, order);
        this.children = this.children.subList(0, order+1);

        // create new innernode
        InnerNode newNode = new InnerNode(this.metadata, newKeys, newChildren);
        int newPageNum = newNode.getPage().getPageNum();
        sync();
        return Optional.of(new Pair(pushUp, newPageNum));
      }
    }
    sync();
    return Optional.empty();
  ...
  public Optional<Pair<DataBox, Integer>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
    ...
    int lastChildIndex = this.children.size() - 1;
    if (lastChildIndex < 0) {
      // this shouldn't happen ever!
    }

    Optional<Pair<DataBox, Integer>> nextChild;
    while (lastChildIndex <= order*2 && data.hasNext()) {
      BPlusNode currNode = getChild(lastChildIndex); // TODO children not initialized for empty inner node!
      nextChild = currNode.bulkLoad(data, fillFactor);
      if (!nextChild.isPresent()) {
        return Optional.empty();
      }
      this.keys.add(nextChild.get().getFirst());
      this.children.add(nextChild.get().getSecond());
      lastChildIndex++;
    }

    // split this node and finish
    List<DataBox> newKeys = new ArrayList<>();
    List<Integer> newChildren = new ArrayList<>();
    DataBox pushUp = this.keys.get(order);
