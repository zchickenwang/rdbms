...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) throws BPlusTreeException {
      ...
      Optional<Pair<DataBox, Integer>> splitResult = this.root.put(key, rid);
      if (splitResult.isPresent()) {
        List<DataBox> newKeys = new ArrayList<>();
        List<Integer> newChildren = new ArrayList<>();
        newKeys.add(splitResult.get().getFirst());
        newChildren.add(this.root.getPage().getPageNum());
        newChildren.add(splitResult.get().getSecond());
        InnerNode newRoot = new InnerNode(this.metadata, newKeys, newChildren);
        this.root = newRoot;
      }
    }
     ...
     *
     * This method should raise an exception if the tree is not empty at time TODO
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) throws BPlusTreeException {
      ...
      if (data == null) return;
      while(data.hasNext()) {
        Optional<Pair<DataBox, Integer>> splitResult = this.root.bulkLoad(data, fillFactor);
        if (splitResult.isPresent()) {
          List<DataBox> newKeys = new ArrayList<>();
          List<Integer> newChildren = new ArrayList<>();
          newKeys.add(splitResult.get().getFirst());
     ...
     *
     *   dot -T pdf tree.dot -o tree.pdf7
     *
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
      ...
      public BPlusTreeIterator(LeafNode startingLeaf, Optional<DataBox> startingKey) {
        this.currLeafIterator = startingKey.isPresent() ? 
          startingLeaf.scanGreaterEqual(startingKey.get()) : startingLeaf.scanAll();
        this.nextLeaf = startingLeaf.getRightSibling();
      }

      @Override
      public boolean hasNext() {
        return this.currLeafIterator.hasNext() || nextLeaf.isPresent();
      }

      @Override
      public RecordId next() {
        while (!this.currLeafIterator.hasNext()) {
          if (!this.nextLeaf.isPresent()) return null;
          this.currLeafIterator = this.nextLeaf.get().scanAll();
          this.nextLeaf = this.nextLeaf.get().getRightSibling();
        }
        return this.currLeafIterator.next();
      }
    }
}
