...
class LeafNode extends BPlusNode {
  ...
  public Optional<Pair<DataBox, Integer>> put(DataBox key, RecordId rid)
    ...
    // full
      ...
      // move over entries
      for (int m = order; m <= order*2; m++) {
        newKeys.add(this.keys.get(m));
        newRids.add(this.rids.get(m));
      }
      this.keys = this.keys.subList(0, order);
      this.rids = this.rids.subList(0, order);
      
      // build new leaf
      DataBox pushUp = newKeys.get(0);
      LeafNode newLeaf = new LeafNode(this.metadata, newKeys, newRids, this.rightSibling); 
    
      // connect right sibling
      int newPageNum = newLeaf.getPage().getPageNum();
  ...
  public Optional<Pair<DataBox, Integer>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
                                                   float fillFactor)
      throws BPlusTreeException {
    // find fill factor limit
    int order = this.metadata.getOrder();
    int maxSize = (int) Math.ceil(order*2.0 * fillFactor);
    
    Pair<DataBox, RecordId> nextEntry;
    // fill up to limit
    while (data.hasNext() && this.keys.size() < maxSize) {
      nextEntry = data.next();
      if (this.keys.size() > 0 && nextEntry.getFirst().compareTo(this.keys.get(this.keys.size()-1)) <= 0) {
        throw new BPlusTreeException("Keys must be bulkloaded in sorted order");
      }
      this.keys.add(nextEntry.getFirst());
      this.rids.add(nextEntry.getSecond());
    }
    ...
    if (data.hasNext()) {
      ...
      newKeys.add(nextEntry.getFirst());
      newRids.add(nextEntry.getSecond());
  ...
  public static LeafNode fromBytes(BPlusTreeMetadata metadata, int pageNum) {
    ...
    int nextLeaf = buf.getInt();
    Optional<Integer> rightSibling = (nextLeaf == -1) ? Optional.empty() : Optional.of(nextLeaf);
