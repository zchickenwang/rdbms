  ...
  public class RIDPageIterator implements BacktrackingIterator<RecordId> {
	    ...
	    */
	    ...
	    public RIDPageIterator(Page page) {
	        this.page = page;
            this.bitmap = getBitMap(this.page);
            this.byteIndex = 0;
            this.bitIndex = -1;
	        this.byteMark = -1;
	        this.bitMark = -1;
            this.nextRecordId = findNext();
            this.prevRecordId = null;
            this.markedPrevRecordId = null;
	    }

	    private RecordId findNext() {
	        if (!increment()) {
	            return null;
            }
            while (Bits.getBit(this.bitmap[this.byteIndex], this.bitIndex) == Bits.Bit.ZERO) {
                if (!increment()) {
                    return null;
                }
            }
            return new RecordId(page.getPageNum(), (short) (byteIndex*8 + bitIndex));
        }
        ...
        private boolean increment() {
            ...
            if (this.bitIndex == 8) {
                this.byteIndex++;
                if (this.byteIndex >= this.bitmap.length) {
                    return false;
                }
                this.bitIndex = 0;
            }
	    ...
	    public void mark() {
	        ...
	        this.byteMark = this.byteIndex;
            this.bitMark = this.bitIndex;
	    ...
	    public void reset() {
            ...
            this.byteIndex = this.bitMark == 0 ? this.byteMark - 1 : this.byteMark;
	        this.bitIndex = this.bitMark == 0 ? 7 : this.bitMark - 1;
            this.prevRecordId = null;
