...
public class Histogram {
  ...
  public void buildHistogram(Table table, int attribute){
    ...
    for (int i = 0; i < numBuckets; i++) {
      ...
      Bucket<Float> currBucket = new Bucket<>(minValue + i*width, minValue + (i+1)*width + supp);
      buckets[i] = currBucket;
    }

    //4. populate the data using the increment(value) method
    Iterator<Record> iter2 = table.iterator();
    if (width == 0) {
      while (iter2.hasNext()) {
        buckets[numBuckets - 1].increment(quantization(iter2.next(), attribute));
      }
    } else {
      while (iter2.hasNext()) {
        float currInc = quantization(iter2.next(), attribute);
        buckets[bucketIndex(currInc)].increment(currInc);
      }
    }
  }
  ...
  private float [] allEquality(float qvalue){
    ...
    // find bucket and distinct count
    int chosen = bucketIndex(qvalue);
    for (int i = 0; i < numBuckets; i++) {
      result[i] = i == chosen ? 1 / ((float) buckets[i].getDistinctCount()) : 0;
    }
  ...
  private float [] allNotEquality(float qvalue){
    ...
    int chosen = bucketIndex(qvalue);
    for (int i = 0; i < numBuckets; i++) {
      result[i] = i == chosen ? 1 - (1 / ((float) buckets[i].getDistinctCount())) : 1;
    }
  ...
  private float [] allGreaterThan(float qvalue){
    ...
    for (int i = 0; i < numBuckets; i++) {
      if (i < chosen) {
        ...
        result[i] = (buckets[chosen].getEnd() - qvalue) / width;
      }
    }
  ...
  private float [] allLessThan(float qvalue){
    ...
    for (int i = 0; i < numBuckets; i++) {
      if (i < chosen) {
        ...
        result[i] = (qvalue - buckets[chosen].getStart()) / width;
      }
    }
