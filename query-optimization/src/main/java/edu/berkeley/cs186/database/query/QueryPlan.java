...
public class QueryPlan {
  ...
  public Iterator<Record> executeOptimal() throws DatabaseException, QueryPlanException {
    ...
    Map<Set, QueryOperator> pass1 = new HashMap<>();
    pass1.put(new HashSet(Arrays.asList(this.startTableName)), minCostSingleAccess(this.startTableName));
    for (String table : this.joinTableNames) {
      pass1.put(new HashSet(Arrays.asList(table)), minCostSingleAccess(table));
    }
    ...
    // been joined.
    Map<Set, QueryOperator> joinTree = new HashMap<>(pass1);
    for (int i = 0; i < this.joinTableNames.size(); i++) {
      joinTree = minCostJoins(joinTree, pass1);
    }
    ...
    this.finalOperator = minCostOperator(joinTree);
    this.addGroupBy();
    this.addProjects();
    return this.finalOperator.execute();
  }
  ...
  public QueryOperator minCostSingleAccess(String table) throws DatabaseException, QueryPlanException {
    ...
    for (int i : getEligibleIndexColumns(table)) {
      PredicateOperator arg1 = this.selectOperators.get(i);
      DataBox arg2 = this.selectDataBoxes.get(i);
      IndexScanOperator indexScan = new IndexScanOperator(this.transaction, table, this.selectColumnNames.get(i), arg1, arg2);
    ...
    return addEligibleSelections(minOp, indexIndex);
  }
  ...
  private Map<Set, QueryOperator> minCostJoins(Map<Set, QueryOperator> prevMap,
    ...
    for (Set tables : prevMap.keySet()) {
      ...
      for (int i = 0; i < this.joinTableNames.size(); i++) {
        ...
        Set nextTables = new HashSet(tables);
        String leftTable = getJoinLeftColumnNameByIndex(i)[0];
        String leftCol = getJoinLeftColumnNameByIndex(i)[1];
        String rightTable = getJoinRightColumnNameByIndex(i)[0];
        String rightCol = getJoinRightColumnNameByIndex(i)[1];
        QueryOperator leftOp = null;
        QueryOperator rightOp = null;
        QueryOperator nextJoinOp = null;
        /**
        ...
        if (tables.contains(leftTable) && !tables.contains(rightTable)) {
          //if (checkTablesContains(tables, leftCol) && this.transaction.getSchema(rightTable).getFieldNames().contains(rightCol)) {
          rightOp = pass1Map.get(new HashSet(Arrays.asList(rightTable)));
          nextJoinOp = minCostJoinType(prevMap.get(tables), rightOp, leftCol, rightCol);
          nextTables.add(rightTable);
          //}
        }
         ...
         */
          ...
          //if (this.transaction.getSchema(leftTable).getFieldNames().contains(leftCol) && checkTablesContains(tables, rightCol)) {
          leftOp = pass1Map.get(new HashSet(Arrays.asList(leftTable)));
          nextJoinOp = minCostJoinType(leftOp, prevMap.get(tables), leftCol, rightCol);
          nextTables.add(leftTable);
          //}
        }
         ...
         */
        if (nextJoinOp == null) continue;
        int cost = nextJoinOp.estimateIOCost();
        if (!map.containsKey(nextTables) || map.get(nextTables).estimateIOCost() > cost) {
          map.put(nextTables, nextJoinOp);
        }
      }
    }
  ...
  private boolean checkTablesContains(Set tables, String col) throws DatabaseException {
    boolean good = false;
    for (Object t : tables.toArray()) {
      good = good || this.transaction.getSchema((String) t).getFieldNames().contains(col);
    }
    return good;
  }
